...
const LineChart = () => {
  ...
  useEffect(() => {
    const fetchSchoolYears = async () => {
      try {
        ...
        const sortedSchoolYears = response.data
          .map((data) => ({
            _id: data._id,
          ...
          .sort(
            (a, b) => b.syStartYear - a.syStartYear || b.syEndYear - a.syEndYear
          );
  ...
  const summary = () => {
    ...
    const schoolYearLabel = schoolYears.find(
      (year) => year._id === selectedSchoolYear
    );
    ...
    function generateMessage(
      ...
      const dateList =
        dateInfo.length === 1 ? dateInfo[0] : dateInfo.join(", ");
      return isAllMonth
        ? `The ${dateLabel}${
            dateInfo.length > 1 ? "s" : ""
          } of ${dateList} in the School Year ${schoolYearText} had the largest number of medical examination/s, with ${
            monthData.count
          } count/s in ${typeLabel}.`
        : `The ${dateLabel}${
            dateInfo.length > 1 ? "s" : ""
          } of ${selectedMonth} ${dateList} in the School Year ${schoolYearText} had the most medical examination/s, with ${
            monthData.count
          } count/s in ${typeLabel}.`;
    }
    ...
    const monthData =
      selectedMonth === "All"
        ? calculateHighestMonthData(selectedType)
        : calculateHighestDayData(selectedType);
    ...
    data.forEach((typeData) => {
      if (selectedType === "All" || typeData.id === selectedType) {
        typeData.data.forEach((dataPoint) => {
          ...
          dayCounts[monthYearDay] =
            (dayCounts[monthYearDay] || 0) + dataPoint.y;
        });
      }
    });
  ...
  const fetchData = async () => {
    try {
      ...
      const selectedYearStart =
        schoolYears.find((year) => year._id === selectedSchoolYear)
          ?.syStartYear || new Date().getFullYear();
      const selectedYearEnd =
        schoolYears.find((year) => year._id === selectedSchoolYear)
          ?.syEndYear || new Date().getFullYear();
              ...
              for (let month = startMonth; month <= endMonth; month++) {
                if (month === selectedMonthIndex) {
                  for (
                    ...
                    const monthYearDayKey = `${year}-${month
                      .toString()
                      .padStart(2, "0")}-${day.toString().padStart(2, "0")}`;
                    schoolYearDays.push({
                      x: monthYearDayKey,
