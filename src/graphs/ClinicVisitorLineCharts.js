...
const ClinicVisitorLineChart = () => {
  ...
  const schoolYearText = schoolYearLabel
    ...
    function generateMessage(monthData, typeData, schoolYearText, selectedMonth, selectedType) {
      ...
      return isAllMonth
        ? `The ${dateLabel}${dateInfo.length > 1 ? 's' : ''} of ${dateList} in the School Year ${schoolYearText} had the largest number of clinic visit/s, with ${monthData.count} count/s in ${typeLabel}.`
        : `The ${dateLabel}${dateInfo.length > 1 ? 's' : ''} of ${selectedMonth} ${dateList} in the School Year ${schoolYearText} had the most clinic visit/s, with ${monthData.count} count/s in ${typeLabel}.`;
    }
          ...
          <Paper elevation={3}>
            <Box p={3}>
              <Typography variant="h4" gutterBottom>
                Monthly/ Daily Clinic Visit/s
              </Typography>
              <Typography variant="body1" paragraph>
                Tracks frequency of clinic visits per School Year, Month, and Type flagging unusually
                high or low visits.
              </Typography>
